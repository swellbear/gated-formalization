# Quantitative Evidence Rubric

**Use before** admitting material that would establish or refute a **numerical / risk-adjusted bar** (C≥H, Sharpe vs B&H, seasonality gap, etc.). Domain-agnostic. Attach to or precede the relevant `04` / evidence intake.

**Date:**  
**Application:**  
**Bar under test (quote freeze):**  
**Source / artifact:**  

---

| # | Check | Result |
|---|-------|--------|
| 1 | **Sample definition** — series, window, return/quantity concept stated? | Yes / Partial / No — |
| 2 | **Costs / taxes / frictions** — included, excluded, or N/A (state which)? | Included / Excluded / N/A — |
| 3 | **Significance vs point estimate** — test/interval, or point estimate only? | Test / Point only / N/A — |
| 4 | **Matched comparison** — same locks / same instance on both sides of the bar? | Yes / Partial / No — |
| 5 | **Sensitivity to sample window** — noted, tested, or untested? | Noted / Tested / Untested — |

---

## Already-included legs (mandatory for numerical / workbook bars)

List legs **already in the base construction** so “what about X?” does not reopen a settled mix. Same spirit as scenario already-included legs.

| Leg | In base bar test? | Notes |
|-----|-------------------|-------|
|     | Y/N | |

**Examples (adapt):** seasonality gap already counted; T-bill / cash yield in strategy sleeve; stated tax/cost friction; matched H and C on same episode.

**If asked “what about X?”:** Point here if X is listed — do not treat as an omission.

---

**Conflicted-source?** (link to E_Package intake if used): Non-conflicted / Conflicted —  

**Bar decision supported by this artifact?** Establish / Not establish / Refute / HOLD — explain:  

**Comparability note:** Same rubric used for markets and fiscal numerical bars so fails remain comparable.

---

*Standing rule: Quantitative evidence quality bar + already-included legs.*
