...
| `learning_lane_15m` | 15-minute Kalshi learning lane (`KXBTC15M`) | `LEARNING LANE` | Pages export published. First fill `KXBTC15M-26SEP071445-45` is settled `paper_win` (Kalshi result=yes, finalized). No 15m charts. |

The 15-minute lane publishes the settled first fill only. Charts stay `not yet available`
and are not invented. The golf WC1 FAIL is a golf record and does not transfer here.
