...
- **J.** `J_farm_promote`: live selecting L1 is closed (not mid-look) and a farm keeper is queued. CoS assigns Lab to date the `declared_at` head (not richest pnl) as a PROPOSED with `execution` still false; Operator RUN-ONLY. Stamp J **only after** Lab is assigned.

`needed` is **false** when the watch is healthy AND no new judicial owe AND no clerical arrears AND the desk already has an assigned worker who is not stale AND F is not owed AND H is not owed AND I is not owed AND J is not owed, **or** when the last CoS closeout stamped the same `reason_ids` (a 90s heartbeat, not a doorbell). F / I / J are not silenced by that stamp unless Lab is assigned. H is not silenced by the stamp; it clears when `honer_consult.json` photocopies the freeze.
