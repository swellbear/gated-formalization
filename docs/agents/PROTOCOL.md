...
- **F.** `F_continuation`: desk idle (or CoS with Job `—`), no assigned worker, no un-operated Lab PROPOSED, and no live 15m trial (executing selection still unscored), **and** no farm keeper ready to take the chair (`J_farm_promote`). Favorite L1 PARK is dead. `R-SKIP-COINFLIP` `execution: false` is not live. Stamp F handled **only after** desk `Status=assigned` / `lab`.
- **H.** `H_honer_freeze`: honer exam is open (or freeze log has an unphotocopied snapshot), `honer_consult.json` does not yet match that freeze, `consult_enabled` is not true, and the desk is not an assigned worker. Clerical copies the freeze. CoS does **not** assign Lab to retype θ. If both H and F, F still assigns Lab invent (H is not a Lab job). Stamp H is not a Lab assign.
- **I.** `I_farm_open`: hunger loop. Desk idle / CoS blank job, no assigned worker, no sitting un-operated PROPOSED. Unused legal catalog slots, else invent the next kind, else `farm_menu_exhausted`. A seated live trial does **not** pause invent. Stamp I **only after** Lab is assigned. Not Ask-Founder.
- **J.** `J_farm_promote`: live selecting L1 is closed **and parked** (not mid-look; an L1 file is the grade sheet, not an empty chair) and a farm keeper is queued. CoS assigns Lab to date the `declared_at` head (not richest pnl) as a PROPOSED with `execution` still false; Operator RUN-ONLY. Stamp J **only after** Lab is assigned.
- **K.** `K_look_due`: executing selecting look is due (n≥70 and no L1, or L1 exists without Operator PARK/CONTINUE). Rings even if Lab is assigned or farm covering. Look owner is Operator, not Systems. Stamp K only after PARK or CONTINUE. Do not stop PaperWatch at 70.

`needed` is **false** when the watch is healthy AND no new judicial owe AND no clerical arrears AND the desk already has an assigned worker who is not stale AND F is not owed AND H is not owed AND I is not owed AND J is not owed AND K is not owed, **or** when the last CoS closeout stamped the same `reason_ids` (a 90s heartbeat, not a doorbell). F / I / J are not silenced by that stamp unless Lab is assigned. H is not silenced by the stamp; it clears when `honer_consult.json` photocopies the freeze. K is not silenced by the stamp until PARK or CONTINUE.

A CoS turn: session start, **one** assign or one closeout via `decide_cos_action` (`golf-offshoot/src/golf_offshoot/learning_lane_15m/cos_tick.py`), desk + leave-off to committed truth, stamp `last_cos_*` via `stamp_cos_closeout`. Then stop. It does not ping Founder for the Next list. It does ping Founder for arm / cash / keys, golf C2/C4/WC3+, and retune golf θ. Bind is not on that list. Forbidden assigns: score `R-SKIP-COINFLIP`, re-score PARK'd `R-SKIP-2TO1-FAVORITE`, arm, bind, git push to `master`. **Lab is legal:** F_continuation assigns one 15m PROPOSED. I_farm_open assigns Lab to date farm notebooks. J_farm_promote assigns Lab then Operator RUN-ONLY. H_honer_freeze is clerical (freeze photocopy); CoS does not assign Lab to retype θ. CoS does not author the PROPOSED. Operator `lab_proposed` or Critic on a **new** hash first. Zero-objection closeout does not skip a sitting PROPOSED, a starved gym, farm hunger, or a queued keeper (assign Operator or Lab). An open honer freeze is clerical, not a Lab invent.
...
- Lift the series HOLD (opens a new lane; not a second ticker on this ledger)
...
- Assign Lab when `I_farm_open` is owed (date unused farm slots or invent the next kind; not Founder)
- Assign Lab when `J_farm_promote` is owed (date the queued keeper; Operator RUN-ONLY; not Founder; not arm)
- Treat `H_honer_freeze` as clerical (freeze photocopy into `honer_consult.json`); do not assign Lab to retype θ
- Honer consult enable is a file gate (`maybe_sync_and_enable`); it may join only after the **seated** selecting rule's first 70 is scored and that rule is still a keeper. Not hour-close-specific. Do not assign Systems to flip the flag.
- After consult has lived and the factory trial is not live: assign Lab `HONER-FROZEN-REPLACE` (executing row from the freeze). Not Founder. Not arm.
...
10. `lab` — when `F_continuation` is owed (no live trial, no un-operated PROPOSED, no farm keeper ready). One **PROPOSED** cheap test under the invent contract (mechanism catalog, density floor `10/n`, `HONER-FAMILY-AMEND` only after a dead honer exam or catalog exhaust, `HONER-FROZEN-REPLACE` only after consult has lived and hour-close is not live, do not retype freeze θ, do not retune clock minute), paper-only, then `operator`. When `I_farm_open` is owed: date unused legal farm notebooks in `LEARNING_LANE_15M_FARM.json` (execution false) in one fire, else invent the next kind, else write `farm_menu_exhausted`. When `J_farm_promote` is owed: date the `declared_at` keeper as a PROPOSED (`execution` false); Operator RUN-ONLY. Never self-admit. Golf idle does not stop a 15m Lab assign. Honesty / hub fail is `D_hub`, not a Lab veto. `H_honer_freeze` is clerical, not this Lab job. Farm notebooks do not spend `trials_to_date` until one takes the chair. This fold does not arm.
