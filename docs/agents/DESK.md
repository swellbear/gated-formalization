...
## ARM hub (separate track — not the 15m learning lane)

`golf-offshoot/src/golf_offshoot/arm_hub/` + `golf-offshoot/data/arm_hub/`. Paper $500 accumulate. `latest/ARM.flag` absent = PAPER. Unattended `--watch`. Trading **NOT ARMED**. Never port 8765.

## Publish gap (named so nobody assumes Pages is self-maintaining)

The runner exports locally. It does **not** commit or push. The public page is **not** self-maintaining. This ARM-hub fold does **not** publish the 15m hub.
