...
| Field | Value |
|-------|--------|
| Updated | 2026-09-08 16:55 EDT (Lab PROPOSED 02 declared `R-SKIP-2TO1-FAVORITE`; execution false; not scored. Handoff → CoS. Bar still not binding.) |
...
1. **The learning tick order is `lane-15m` → `digest-figures` → `digestor` (caveats only) → `operator` → `systems` → `validator`.** `hub-ui` runs only if the display is wrong or stale. **`illustrator` is owed by the wake** when the PNG lags live journal/settlements by more than one window (one window of trail is the open window and is allowed). **`lab` PROPOSED 02 landed 2026-09-08 16:55 ET** — `R-SKIP-2TO1-FAVORITE` declared (`favorite_odds=2`, skip posted YES ≥ 2/3), `execution=false`, not scored, not a revival of `R-SKIP-COINFLIP`. Desk `Status=done`, next=chief-of-staff. **Do not run Lab in a CoS authoring turn.** Founder saved **15m worker tick** (`golf-offshoot/docs/WORKER_AUTOMATION.md`, cron `7,22,37,52`). **CoS last_cos** handled `A_idle_uncovered_judicial, B_new_judicial, E_idle_unassigned` at `7e15c76` (not restamped this fire). A tick fires on a new official settle, a new paper fill, a pending window clearing, or a stale board. CoS starts when `crew_tick.needed` is true. Do not schedule CoS at 90s.
