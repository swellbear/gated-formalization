...
| Field | Value |
|-------|--------|
| Updated | 2026-09-08 17:32 EDT (CoS assigned operator to answer CRITIC 03 on Turn 3 bar hashes + registry `R-SKIP-2TO1-FAVORITE`. Bar still not binding. HOLD stands.) |
...
1. **The learning tick order is `lane-15m` → `digest-figures` → `digestor` (caveats only) → `operator` → `systems` → `validator`.** `hub-ui` runs only if the display is wrong or stale. **`illustrator` is owed by the wake** when the PNG lags live journal/settlements by more than one window (one window of trail is the open window and is allowed). **`lab` PROPOSED 02 landed 2026-09-08 16:55 ET** — `R-SKIP-2TO1-FAVORITE` declared (`favorite_odds=2`, skip posted YES ≥ 2/3), not a revival of `R-SKIP-COINFLIP`. **Operator 17:11 ET RUN-ONLY**; `execution=true` (paper only); not scored; `R-SKIP-COINFLIP` untouched. Artifact: `LEARNING_LANE_15M_OPERATOR_NOTE_PROPOSED_02.md`. **Soften Critic 17:40 ET CRITIC 03** (`LEARNING_LANE_15M_EVIDENCE_BAR_CRITIC_03.md`): six UPHELD on Turn 3 bar hashes + the executing registry row; bar not edited; not scored. **CoS 17:32 ET assigned operator** to answer it; `last_cos` handled `A_worker_done`. **Do not run Operator in a CoS authoring turn.** Worker tick owns the answer. Founder saved **15m worker tick** (`golf-offshoot/docs/WORKER_AUTOMATION.md`, cron `7,22,37,52`). A tick fires on a new official settle, a new paper fill, a pending window clearing, or a stale board. CoS starts when `crew_tick.needed` is true. Do not schedule CoS at 90s.
