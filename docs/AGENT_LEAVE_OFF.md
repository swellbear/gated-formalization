...
7. **Evidence bar has been attacked, amended, and is still NOT binding.** Read `LEARNING_LANE_15M_EVIDENCE_BAR_CRITIC_01.md` then `LEARNING_LANE_15M_EVIDENCE_BAR_OPERATOR_ANSWER_01.md` before touching the bar. Operator sustained 7 objections outright and 7 in part; **none was dismissed**. Amendments: null is now `mean(d) ≤ δ` not `≤ 0`; α is `0.05/(k(k+1))`, which brings family-wise error from 15.2% to 4.6%; **n moved 40 → 70** with sd at its 95% upper bound 0.784, which moved δ/MDE from **0.997 to 1.525**; a seeded permutation control at fixed skip count; the power curve is on the bar's face; the fee section is rewritten as *omitted costs*; and `critic-invariants` passing is now a **third binding condition**.
   - **Do not score `R-SKIP-COINFLIP`.** n is now 70, not 40. Report the eligible count only; do not open a post-declaration outcome file to get it.
   - **The band is not verifiably pre-registered.** The RUN-ONLY note published 24 marks *and* their pnl at 21:49:37 on 09-07; the (0.45, 0.55) band was first committed **8h 07m later**, and 8 of those 24 marks fall inside it. The bar now records this and scopes L1 accordingly.
   - **Why it is still not binding:** `critic-invariants` reports `passed: false`. `fee_schedule_hash_recorded` fails because the fetch of the Kalshi schedule returned **HTTP 429** at 12:00 ET. Retry and pin the real digest. **Nobody may write a placeholder to turn that check green** — Operator declined, CoS declined. Founder read-once has also not happened.
   - **Established is currently unreachable and the bar now says so.** Nothing outside the test suite calls `rules.decide()`; `paper.py` fills every candidate window. The bar names the `paper.py` change as an explicit precondition and states the permitted flip point.
