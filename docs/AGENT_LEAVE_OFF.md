...
| Field | Value |
|-------|--------|
| Updated | 2026-09-09 10:36 EDT (CoS assigned soften-critic to attack ANSWER 11 amended hashes. Two SUSTAINED stand: scorer named as replay-clock interval not Lived; missing-fields default disclosed. Not scored. Binding false. HOLD stands. next=soften-critic.) |
...
7. **Evidence bar has been attacked eleven times and is still NOT binding.** Soften Critic **CRITIC 11** (`LEARNING_LANE_15M_EVIDENCE_BAR_CRITIC_11.md`, 09:37 ET) attacked the Systems 2026-09-09 wiring hashes (`61E2D677…` / `F34DC187…` / `BBD87E52…`). Two UPHELD: the scorer's `evidence=lived` stamp is a clock label (interval gate closed; Lived definition not checked); missing `replay_close_*` defaults every OOS close to lived, so `R-SKIP-COINFLIP` L1 would be stamped lived. **Operator 10:35 ET ANSWER 11** (`LEARNING_LANE_15M_EVIDENCE_BAR_OPERATOR_ANSWER_11.md`): two SUSTAINED; face/JSON/registry now say the scorer enforces the replay clock interval, not Lived; `evidence=lived` token means `closed > lived_paper_begins_at` and does not mean the loop honoured `decide()`; `lived_replay_enforced_by_scorer` set false; missing-fields default disclosed. Not scored; not bound. Systems 2026-09-09 09:05 EDT wired lived/replay into `score_rule` (reject `(16:53, 17:11]` as in-interval; `decide()` still keys `declared_at`). Prior: CRITIC 09 and CRITIC 10 both zero UPHELD on the ANSWER 08/09 hashes. Condition 2 still fails on findings that do not cover these bytes and on the unpinned fee hash (HTTP 429 at 13:42; `schedule_sha256` empty; no placeholder). Condition 3 (`founder_read_once`) is unmet and is **not requested**. next=soften-critic.
   ...
   - **Why it is still not binding:** condition 1 is unmet on the ANSWER 11 amended bytes (new hashes; this pair answers the Systems hashes, not these bytes). Condition 2 fails on findings that do not cover these bytes and on the unpinned fee hash (HTTP 429 at 13:42; `schedule_sha256` empty; no placeholder). Condition 3 (`founder_read_once`) is unmet and is **not requested**. next=soften-critic.
