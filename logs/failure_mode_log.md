...
- **Direction of error:** **method-pressure-point** — a stalled invent→test / solve-loop string is easy to leave in pulses and then jump to a new claim, so the taught/foggy split vanishes. Digestion parks the unfinished leftover (vehicle+cutoff fragility; 2023 year-break HARDENED toward real) and refuses elevation. This write-up is **not** a license to invent.
- **Which rule or judgment contributed:** [`docs/DIGESTION_HABIT.md`](../docs/DIGESTION_HABIT.md); invent paused unless Founder/user locks a new named-gap string; Amb HARDEN ≠ skill-met; FRAGILE ≠ elevated; Track B ≠ F-SKILL; do not treat a scoped survivor as a null.
- **Adjustment made (if any):** None to parent Amb math (held 1.0). Docs-only digestion. No new invent authorized.
- **Notes:** Live scoped horse stays Brent **H-SPOT-MOY-CONT** (**FRAGILE**). Open residual: fragility/vehicle Amb (year-stability slice HARDENED). P3=B held. Invent paused.
