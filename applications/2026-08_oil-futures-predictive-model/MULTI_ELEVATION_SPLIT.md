...
*Rank 4 locked 2026-08-17. **D-EXIST established (futures-target only).** **L-SESS** in force. **H-GAP-FADE** small F-DAY (does **not** promote). **L-HUNT-PRETELL** scored (no discovery survivor). **H-SPARSE-CAL** / **H-SPARSE-VOL** scored (neither promotes). **H-LAG-WF** scored (F-CC loss; does **not** promote). **H-KS-FTS** not run. **V-COST-V2** named. **L-SCREEN-Y-PROMOTE** in force. F-SKILL and V-VALUE not established. Closeout: **hard stop (residuals live)**. Not a trade. Next: `leave skill not shown` · `name horse …` (**different**; do **not** remix gap horses) · `leave screen rule`.*
