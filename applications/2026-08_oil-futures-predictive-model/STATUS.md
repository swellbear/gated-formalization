...
Closeout remains **hard stop (residuals live)**. The skill class is now **named** (CME official CL open and settlement). The test **did not run**: we could not obtain that tape and did not substitute Yahoo. Night/day/whole-trip skill is still **not shown**. After-cost value is still **not shown**. This is not trading advice.
...
| **Phase** | Named-class pulse **L-PULSE-TAPE-0** (executed; bars not met) |
| **Amb** | **2.5** (**≠ clearance**) |
| **Locks in force** | Rank 4; D-EXIST-MET-FT; V-COST either; V-SRC leave unnamed; **F-SRC-CME-TAPE**; L-SESS; L-PULSE-TAPE-0 |
| **Next authorization needed** | `live CME only` / `stipulate stand-in …` / `leave tape pending` — **not** `none — hard stop` |
...
**Maps:** [MAP_Futures_Target_Forecasting_Methods.md](MAP_Futures_Target_Forecasting_Methods.md) · [MAP_What_Can_Move_CL.md](MAP_What_Can_Move_CL.md) · [MAP_Session_Split.md](MAP_Session_Split.md)  
**Named class / pulse:** [Lock_FSRC_Named_CME_Tape.md](Lock_FSRC_Named_CME_Tape.md) · [PULSE_Baseline_Session_RMSE.md](PULSE_Baseline_Session_RMSE.md)  
**Residuals:** [R-F-SKILL](RESIDUAL_BRANCH_MENU.md#r-f-skill) **pursue** (named; tape missing) · [R-LIVE-STANDIN](RESIDUAL_BRANCH_MENU.md#r-live-standin) **pursue** · [R-F-COMBO](RESIDUAL_BRANCH_MENU.md#r-f-combo) park-until-trigger · [R-V-VALUE](RESIDUAL_BRANCH_MENU.md#r-v-value) park-until-trigger
...
**Gates:** Examination + hygiene + SHARE_PACK + optional-modes **offer** done → **hard stop (residuals live)** because [R-F-SKILL](RESIDUAL_BRANCH_MENU.md#r-f-skill) remains `pursue`. **Amb ≠ clearance.** Leave-unnamed ≠ unlikely. Existence-met ≠ skill-met.
