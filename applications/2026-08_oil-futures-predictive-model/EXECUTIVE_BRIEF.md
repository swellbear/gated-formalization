...
Walk-forward skill versus last settlement is **not shown**. Yahoo is a **stipulated stand-in**. A named Trump Truth Social week/month hunt **failed at discovery** (both **tied** no-change). Overnight-gap fade had a **small day-session** edge; the whole trip was unchanged — **does not promote**. A named eight-horse pretell hunt **failed at discovery**. Two sparse horses were scored. **Neither promotes.** Kearney–Shang **not run**.
...
- Existence of a specified non-no-change mapping for listed crude **futures** (**D-EXIST-MET-FT**)  
- V-COST **V2** (fees + $10/contract/side; not V-VALUE-met)
...
**Keep saying:** A futures-target recipe has been written. Yahoo is a stand-in, not live CME. A small day-session fade is not a whole-trip pass. The DJT Truth hunt failed at discovery. The pretell hunt failed at discovery. The lagged-return horse lost on the whole trip. Paper costs are V2; that is not a value pass. Skill and after-cost value are not shown.

**Test next (only if authorized):** `leave skill not shown` · `name horse …` on Yahoo (different; do **not** add year/6-month/day after scores; do **not** retune the lexicon). Live CME only if the F-CC promotion gate fires.

---

## Residual-branch menu (at closeout)

Offered. Skill leftover **live** ([R-F-SKILL](RESIDUAL_BRANCH_MENU.md#r-f-skill) DJT no survivor; FADE small F-DAY, no promote; pretell hunt failed at discovery; CAL/VOL/H-LAG fail promote; H-KS not run). Live vs stand-in **executed** ([R-LIVE-STANDIN](RESIDUAL_BRANCH_MENU.md#r-live-standin)). After-cost book parked ([R-V-VALUE](RESIDUAL_BRANCH_MENU.md#r-v-value)). Combo parked ([R-F-COMBO](RESIDUAL_BRANCH_MENU.md#r-f-combo)). [`RESIDUAL_BRANCH_MENU.md`](RESIDUAL_BRANCH_MENU.md)

---

## Optional modes (offer ≠ run)

[`OPTIONAL_MODES_MENU.md`](OPTIONAL_MODES_MENU.md) — UX/CX/CR **declined, not run** (operator **C**). QI **N/A** (no failed numerical instance bar; stand-in baseline RMSE is not a QI path).
...
| Amb | **1.0** |
| Locks / proxy IDs | Rank 4; D-EXIST-MET-FT; **V-COST-V2**; **L-SCREEN-Y-PROMOTE**; **L-HUNT-DJT**; F-SRC-CME-TAPE; L-STANDIN-Y-CLF; **H-LAG-WF**; **H-SPARSE-CAL**; **H-SPARSE-VOL**; L-SESS; V-SRC leave unnamed |
...
*Plain verdict first; Live vs stand-in when applicable; no silent softening.*
