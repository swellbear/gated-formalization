#!/usr/bin/env python3
"""Named finite discovery/confirm annual-season overlay on Yahoo CL.

Protocol frozen in Lock_Hunt_CL_Seas.md *before* last-500 confirm scores.

Two horses: H-CL-SEAS-ANN (lags + sin/cos day-of-year) and
H-CL-SEAS-MON (lags + month dummies, January baseline).
Pick one on discovery F-CC only if it strictly beats 0.

Does not download. Does not treat Yahoo as CME. Not a trade.
Not a Track B spot hit-rate horse. Not H-SPARSE-CAL event-day sparse.
"""
...
YEAR_LEN = 365.25
HORSES = [
    {"id": "H-CL-SEAS-ANN", "kind": "fourier"},
    {"id": "H-CL-SEAS-MON", "kind": "month"},
]
...
def session_returns(rows: list[dict]) -> list[dict]:
    ...
    for i in range(1, len(rows)):
        ...
        d = date.fromisoformat(cur["date"])
        doy = d.timetuple().tm_yday
        rec["sin"] = math.sin(2.0 * math.pi * doy / YEAR_LEN)
        rec["cos"] = math.cos(2.0 * math.pi * doy / YEAR_LEN)
        rec["month"] = d.month
        rec["month_dummies"] = [1.0 if d.month == m else 0.0 for m in range(2, 13)]
        out.append(rec)
    return out
...
def extra_features(spec: dict, rec: dict) -> list[float]:
    if spec["kind"] == "fourier":
        return [float(rec["sin"]), float(rec["cos"])]
    return [float(v) for v in rec["month_dummies"]]
    ...
    for hid, r in beat:
        if r == best:
            return {
                ...
                "reason": "lowest discovery F-CC RMSE among horses that strictly beat 0; ties keep H-CL-SEAS-ANN",
            }
...
def main() -> int:
    ...
    p.add_argument("--out", default=str(here / "data" / "cl_seas_hunt_scores.json"))
    args = p.parse_args()
