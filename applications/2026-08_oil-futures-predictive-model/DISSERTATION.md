...
**Method path:** Phase 1 only. … → **L-HUNT-SPOT-TREND**; pulse **L-PULSE-SPOT-1** (no survivor) → **L-HUNT-SPOT-INV**; pulse **L-PULSE-SPOT-INV-1** (no survivor either board) → **L-HUNT-SPOT-CROSS**; pulse **L-PULSE-SPOT-CROSS-1** (WTI fail; Brent confirm tiny 250; F-SKILL parked). No Phase 2.
...
| Spot 21-day sign vs continuation (Track B) | **not established** (**L-PULSE-SPOT-1** / **INV-1** no survivor; **CROSS-1** WTI fail / Brent confirm tiny 250 ≠ met; not a refute of every trend rule) |
...
Stand-in baseline, **H-LAG-WF**, **H-SPARSE-CAL**, **H-SPARSE-VOL**, **L-HUNT-PRETELL**, **H-GAP-FADE** / **H-GAP-CONT**, **L-HUNT-DJT**, **L-HUNT-COT**, **L-HUNT-SPOT-TREND**, **L-HUNT-SPOT-INV**, **L-HUNT-SPOT-CROSS** — **not** bar-met. Paper costs **V2**. No named book, so V-VALUE still unused.
...
**Continuation / hard-stop note:** Hygiene sealed. Optional modes declined. Skill leftover stays **live** ([R-F-SKILL](RESIDUAL_BRANCH_MENU.md#r-f-skill)). Spot-trend drawer **executed** ([R-SPOT-TREND](RESIDUAL_BRANCH_MENU.md#r-spot-trend); not established). Tape leftover **executed** as stand-in ([R-LIVE-STANDIN](RESIDUAL_BRANCH_MENU.md#r-live-standin)); reopen live **only if** **L-SCREEN-Y-PROMOTE** fires. Phase 2 not entered. Next: `leave skill not shown` · queued **C-SPOT-LOGIT** · `name horse …` (**different** CL recipe) · `leave screen rule`. **Do not** auto-open DataMine.
