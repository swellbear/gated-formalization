...
| ID | Status / freeze |
|----|-----------------|
| [R-F-SKILL](RESIDUAL_BRANCH_MENU.md#r-f-skill) | Skill class named; **L-HUNT-DJT** no survivor; **H-GAP-FADE** small F-DAY / F-CC **tie**; **L-HUNT-PRETELL** no survivor; **H-SPARSE-CAL** tiny 500 / **fails** 750; **H-SPARSE-VOL** and **H-LAG-WF** F-CC **loss**; H-KS not run; **not established**. `pursue`. Reopen: **different** horse on Yahoo (do **not** add year/6-month/day after DJT scores; do **not** retune the lexicon) |
...
| `name horse …` | Score a named **front-only** recipe on Yahoo under **L-SCREEN-Y-PROMOTE** | Does not auto-meet V-VALUE; Yahoo win ≠ live clearance; do **not** add year/6-month/day after DJT scores; do **not** retune the lexicon |
...
**Default if no further authorization:** keep original wording + **hard stop (residuals live)** with this assessment. Skill leftover stays live (DJT no survivor; FADE small F-DAY, no promote; pretell hunt failed at discovery; H-LAG lost on F-CC; H-KS not run).
...
Awaiting further authorization (`leave skill not shown` · `name horse …` (**different**; do **not** add year/6-month/day after scores; do **not** retune the lexicon) · `leave screen rule`). Live CME **only if** the F-CC gate fires. Optional modes **declined**.
