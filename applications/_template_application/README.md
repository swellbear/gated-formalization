...
Copy this entire folder to `applications/YYYY-MM_short-name/` for each new run.
...
- `DISSERTATION.md` + **`EXECUTIVE_BRIEF.md`** + **`SHARE_PACK.md`** — mandatory at closeout (share pack = partner front door)
- **`STATUS.md`** — Plain status + **Closure state** (endpoint vs hard stop)
- **`PLAIN_LANGUAGE_CARD.md`** — dual-audience asks; reply grammar (below)
...
- `run UX` / `decline UX`
- `run QI` / `run CX` / `run CR` (when those modes apply)
...
- `RESIDUAL_BRANCH_MENU.md` at closeout when residuals remain (offering ≠ running; classify + menu)
