...
**Last check:** 2026-09-05 — docs-only string **CLOSE**. Honest **KILL** of claim *BIA wearable/sensor = accurate weight (bathroom or farm scale)* (~11 kg / ~25 lb RMSE). Ship-A (#57) stays method-practice demo (dry-run **75.2460** kg, synthetic). S1 HARDEN (#56) kept as holdout Amb bite. S2 Soften/park (MISS). S3 HOLD/park (not run). Animal parks stay. Lab **HOLD**. Rithm **archive only**. Digestion: [`DIGESTION_STRING_CLOSE.md`](DIGESTION_STRING_CLOSE.md).

**What this is not:** This is **not** rithm (archive only). Livestock transfer is **not** claimed. Farm livestock continuous-weighing is a **separate** Amb. Farm / commercial / bathroom-scale weighing is **not** solved. Residual **~11 kg / ~25 lb** RMSE is **not** a product win. **Not clinical.** Step-1 SUCCEED is **not** a trained commercial map. S1 HARDEN / Ship-A is **method-practice only**. DUA: statistical analysis / **no re-id**. Example is synthetic. Do **not** invent rows. Do **not** write skill-met / elevated language for farm weighing. Do **not** invent a new Amb.

**Process:** Lab invented hunt probes, then a cheap sklearn holdout, then a method-practice ship. Operator admits, rejects, or parks. Lab does **not** self-admit. After this CLOSE fold Lab is **HOLD**.
...
`method-practice S1-B predict package (Ship-A)` → kill vs land: dry-run example prints **75.2460** kg from the admitted S1-B pipeline vs a broken or over-claimed ship → last check: 2026-09-05 Operator **ADMIT Ship-A** (#57) under [`ship/`](ship/) (`StandardScaler`+`RidgeCV`; n_train 3420 / n_test 856; alpha 3.1622776601683795; holdout RMSE 11.271495752140735 / MAE 8.783979696023593 / R² 0.7363714213498394; fit_note train-split only matching HARDEN; example synthetic) → status: **hardened** / **ADMITTED** (method-practice demo only; ~11 kg / ~25 lb RMSE ≠ scale; not clinical; no livestock transfer; DUA statistical / no re-id)

`BIA wearable/sensor = accurate weight (bathroom or farm scale)` → kill vs harden: holdout RMSE at bathroom-scale / farm-scale accuracy vs leftover ~11 kg / ~25 lb error → last check: 2026-09-05 S1-B test RMSE **11.271 kg ≈ 24.85 lb**; string **CLOSE**; claim **KILL** — a scale that misses by ~25 lb is not an accurate bathroom or farm scale; S1 HARDEN (BIA beyond anthro) is **not** this claim → status: **killed** (docs-only CLOSE; ship stays method-practice demo; animal parks stay; Lab HOLD; rithm archive only)
