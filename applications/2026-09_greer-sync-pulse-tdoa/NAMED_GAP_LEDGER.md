...
**Process:** Lab invents; Operator admits, rejects, or parks. Lab does **not** self-admit. After DRIFT1, next named pulse is **GATE1**, then a Greer-facing write-up (not this fold). Still **no RF / ML**. **No** fingerprint rescue. **Multipath later.**
...
`hardware X` → kill vs succeed: a later gate names a hardware / spectrum path that can carry its own **X** vs stay parked while the live path is sim-only → last check: 2026-09-05 Operator **PARK** hardware **X** (DRIFT1 HARDEN does **not** unpark it) → status: **paused** / **PARKED**
...
`GATE1 pulse` → kill vs harden: named next after DRIFT1 — then a Greer-facing write-up → last check: none (named; not run) → status: **open** / **named** (not this fold)
