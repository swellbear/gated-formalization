...
**Last check:** 2026-09-05 — Operator **ADMIT #0 HARDEN** (after fog peek C1/C2/C3 SUCCEED). Under ideal simultaneous sync + Gaussian Δt only, planar TDOA geometry with frozen Chan (1994) 2D WLS is **not** the bottleneck. Provisional **sim X = 0.50 m** locked (**median**-based @ 1 ns; **median-not-p90**; 1 ns p90 ≈ **1.16 m**). Hardware **X PARKED**. Next pulse **named** (not run): multipath / NLOS positive-bias injection under the same frozen Chan. Score: [`SCORE_GEOMETRY_0.md`](SCORE_GEOMETRY_0.md). Digestion: [`DIGESTION_GEOMETRY_0.md`](DIGESTION_GEOMETRY_0.md). Fog peek record: [`DIGESTION_FIRST_PULSE.md`](DIGESTION_FIRST_PULSE.md).
...
**Process:** Lab invents; Operator admits, rejects, or parks. Lab does **not** self-admit. After #0, the next pulse is **named** (multipath / NLOS positive-bias injection, same frozen Chan) and is **not run** this fold. Still **no RF / ML**.
...
`multipath in a GPS-denied box` → kill vs succeed: this is standing honesty, not a hunt — a denied box is a hard radio environment; do **not** silently drop multipath → last check: 2026-09-05 **C3 SUCCEED** (scoring poseable) then **#0 did not inject** multipath — leftover honesty stays open → status: **open** (constraint; named next pulse = multipath / NLOS positive-bias injection, same frozen Chan; **not run**)

`multipath / NLOS positive-bias injection (same frozen Chan)` → kill vs harden: a later pulse that injects positive NLOS / multipath bias under the **same** Chan 1994 2D WLS and shows whether the **median** 0.50 m bar survives vs a pulse that is not run → last check: 2026-09-05 **named** as next pulse; **not run** this fold → status: **open** / **named next, not run**
