...
## What this string must do next (later the same day: abstract ingest)
...
**Lab HOLD invent** except the locked **A2 → A3 → A4** suite (later **A1 Soften**). Write-up is **on disk** and is **sync-fragility evidence only**: Founder-polished [`GREER_WRITEUP.md`](GREER_WRITEUP.md) is **PRIMARY** — **HOLD send** until the suite digests. Lab audit stays on [`GREER_WRITEUP_DRAFT.md`](GREER_WRITEUP_DRAFT.md). Still **no RF / ML**. Do **not** invent a fingerprint rescue. Do **not** treat GATE1 as a repair. **Multipath later.** Hardware **X** stays **PARKED**.

## What stays parked / closed

- **Hardware X** stays **PARKED**.
- **Send to Greer** stays **HOLD** until the A2→A3→A4 suite digests. Prior write-up = sync-fragility evidence only.
- **Lab invent** stays **HOLD** except the locked suite (later **A1 Soften**).
...
This note does **not** authorize sending the write-up. It does **not** show a TDOA locator. It does **not** start training. It does **not** unpark hardware **X**. It does **not** invent A2–A4. **A1** is opened on the ingest fold, not here.
