...
**A1 Soften** on the record (Kill not triggered). Absolute **≤1 m** poseable under DGPS-class RN survey error (Chan abs **0.832 m** @ `σ_ref=0.5 m`, `σ_t=1 ns`). Absolute **≤0.50 m not** under RN floor (**0.691 / 0.832** at 0.4 / 0.5). **X = 0.50 m** stays **perfect-ref scoped sim only**. JOINT1 scrape abs **0.449** rides the floor — **do not Harden absolute X**. Patent-facing **≤1 m xy** stands. **DGPS ~0.4–0.5 m** absolute floor named. Silent GEOM0 absolute-≤0.50 **Softened**. Abstract ingest **ADMITTED** as Amb spine ([`SOURCE.md`](SOURCE.md)). Prior SYNC/JOINT/DRIFT/GATE = **sync-fragility evidence (partial)**. **GEOM0 HARDEN** stands (perfect-ref). Next: **GO A2** then **A3** then **A4**. Link/map **PARKED**. First-pulse fog peek **ADMITTED** (C1/C2/C3 SUCCEED). Founder **CLAIM LOCK** recorded. Honesty: **path-shared batch**, **not** free per-epoch realtime. **Not** multipath-robust. **Not** hardware. **PARK** hardware **X**. Greer-facing write-up **on disk** = **sync-fragility evidence only**: Founder [`GREER_WRITEUP.md`](GREER_WRITEUP.md) is **PRIMARY** — **HOLD send** until the suite digests. Lab audit: [`GREER_WRITEUP_DRAFT.md`](GREER_WRITEUP_DRAFT.md). **Lab HOLD invent** except the locked suite. Multipath later.
...
**Bars (after abstract ingest + A1 Soften):** patent-facing **≤1 m xy** **stands** (poseable under DGPS-class RN survey error; **not** claim copy). **Sim X = 0.50 m** remains (**median** @ 1 ns RX noise) only under **perfect-known refs + mild NLOS + `σ_sync ≲ 3 ns` under JOINT1 (fixed offsets) + named DRIFT1 batch α + GATE1 refuse-belt** — **not** a patent promise and **not** an absolute bar under the RN floor. **X is median-not-p90** (1 ns p90 ≈ **1.16 m**). **DGPS ~0.4–0.5 m** named absolute floor. **Hardware X PARKED**. **Not** free per-epoch realtime.
...
Remaining live leftover is **A2** (clock-count / TDOA-resolution), then **A3**, then **A4**. **A1 Soften** is scored. Link/map **PARKED**. Our **MULTIPATH1** leftover stays **later** (not A3). Prior GATE1 is scored as **partial** sync-fragility evidence.
...
Operator **ADMIT Soften**. Kill **not** triggered. RN survey error on frozen Chan. Absolute ≤1 m poseable under DGPS-class RN survey error (Chan abs median **0.832 m** @ `σ_ref=0.5 m`, `σ_t=1 ns`). Absolute ≤0.50 m **not** under RN floor (**0.691 / 0.832** at 0.4 / 0.5). **X = 0.50 m** stays **perfect-ref scoped sim only**. JOINT1 scrape abs **0.449** @ `σ_ref=0.5` / `σ_sync=0` rides the floor — **do not Harden absolute X**. Silent GEOM0 absolute-≤0.50 **Softened**. Next: **GO A2** then **A3** then **A4**. See [`SCORE_A1.md`](SCORE_A1.md).

## Greer-facing write-up (on disk; sync-fragility evidence only; HOLD send)

Founder-polished [`GREER_WRITEUP.md`](GREER_WRITEUP.md) is **PRIMARY**. It is **sync-fragility evidence only**. **HOLD send** until the **A2 → A3 → A4** suite digests. Lab audit: [`GREER_WRITEUP_DRAFT.md`](GREER_WRITEUP_DRAFT.md). **Lab HOLD invent** except the locked suite. Still **no RF / ML**. **Multipath later.** Fog peek + later gates: [`PROPOSED_PULSE.md`](PROPOSED_PULSE.md).

## Reading order

1. [`STATUS.md`](STATUS.md) — where we are
2. [`SCORE_A1.md`](SCORE_A1.md) — A1 gated metrics (RN floor)
3. [`DIGESTION_A1.md`](DIGESTION_A1.md) — what A1 taught
4. [`ABSTRACT_INGEST_SUMMARY.md`](ABSTRACT_INGEST_SUMMARY.md) — gated ingest
5. [`SOURCE.md`](SOURCE.md) — published abstract (not claims)
6. [`DIGESTION_ABSTRACT_INGEST.md`](DIGESTION_ABSTRACT_INGEST.md) — what ingest taught
7. [`GREER_WRITEUP.md`](GREER_WRITEUP.md) — Founder-polished send file (**sync-fragility evidence only**; **HOLD send** until suite digests)
8. [`GREER_WRITEUP_DRAFT.md`](GREER_WRITEUP_DRAFT.md) — Lab audit draft
9. [`NAMED_GAP_LEDGER.md`](NAMED_GAP_LEDGER.md) — the open / parked / hardened / Soften lines
10. [`COPY_GATE.md`](COPY_GATE.md) — no claim-language product copy
11. [`PROPOSED_ABSTRACT_INGEST.md`](PROPOSED_ABSTRACT_INGEST.md) — ingest gate
12. [`SCORE_GATE1.md`](SCORE_GATE1.md) — GATE1 gated metrics (partial sync-fragility)
13. [`DIGESTION_GATE1.md`](DIGESTION_GATE1.md) — what GATE1 taught
14. [`SCORE_DRIFT1.md`](SCORE_DRIFT1.md) — DRIFT1 gated metrics
15. [`DIGESTION_DRIFT1.md`](DIGESTION_DRIFT1.md) — what DRIFT1 taught
16. [`SCORE_JOINT1.md`](SCORE_JOINT1.md) — JOINT1 gated metrics (prior Soften)
17. [`DIGESTION_JOINT1.md`](DIGESTION_JOINT1.md) — what JOINT1 taught
18. [`SCORE_SYNC_1.md`](SCORE_SYNC_1.md) — SYNC1 gated metrics (prior Soften)
19. [`DIGESTION_SYNC_1.md`](DIGESTION_SYNC_1.md) — what SYNC1 taught
20. [`SCORE_MULTIPATH1.md`](SCORE_MULTIPATH1.md) — MULTIPATH1 gated metrics (prior Soften)
21. [`DIGESTION_MULTIPATH1.md`](DIGESTION_MULTIPATH1.md) — what MULTIPATH1 taught
22. [`SCORE_GEOMETRY_0.md`](SCORE_GEOMETRY_0.md) — #0 gated metrics
23. [`DIGESTION_GEOMETRY_0.md`](DIGESTION_GEOMETRY_0.md) — what #0 taught
24. [`PROPOSED_PULSE.md`](PROPOSED_PULSE.md) — Lab fog peek + later gates
25. [`DIGESTION_FIRST_PULSE.md`](DIGESTION_FIRST_PULSE.md) — what the peek taught
26. [`DIGESTION_FROM_CELL_TOWER.md`](DIGESTION_FROM_CELL_TOWER.md) — what the last string taught
27. [`RESIDUAL_BRANCH_MENU.md`](RESIDUAL_BRANCH_MENU.md) — decision log
28. [`notes.md`](notes.md) — one-line pointer
