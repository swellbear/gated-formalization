...
**DRIFT1 HARDEN** on the record under the named budget: batch path-shared τ + linear α restores median ≤ **0.50 m** on SYNC1 drift breakers (drift=3 @ `σ=0` → **0.221 m**; drift=10 → **0.223 m**; **α̂ recovers**). **JOINT1 Soften** (`σ_sync ≲ 3 ns` **fixed offsets**) still stands. **GEOM0 HARDEN**, **MULTIPATH1 Soften**, and prior **SYNC1 Soften** (Chan-alone) still stand. First-pulse fog peek **ADMITTED** (C1/C2/C3 SUCCEED). Founder **CLAIM LOCK** recorded. Provisional **sim X = 0.50 m** remains, scoped to **`σ_sync ≲ 3 ns` under JOINT1 (fixed offsets) + named DRIFT1 batch α + mild NLOS** (**median**-not-p90; 1 ns p90 ≈ **1.16 m**). Honesty: **path-shared batch**, **not** free per-epoch realtime. **Not** multipath-robust. **Not** hardware. **PARK** hardware **X**. Next (**not this fold**): **GATE1**, then a Greer-facing write-up. Multipath later.
...
**X:** provisional **sim X = 0.50 m** remains (**median** @ 1 ns RX noise). Honest only under **mild NLOS + `σ_sync ≲ 3 ns` under JOINT1 (fixed offsets) + named DRIFT1 batch α**. **X is median-not-p90** (1 ns p90 ≈ **1.16 m**). **Hardware X PARKED**. **Not** free per-epoch realtime.
...
Remaining live leftover includes **multipath** (later) + **GATE1** (not hyperbolic geometry).

## Eval rules (locked at peek)

- GPS / DGPS, if used at all, **place and time the reference nodes only**. They are **never** the mobile fix.
- Estimator class for the live path is ≥3-reference **simultaneous-sync TDOA** on a **sim-only** path (frozen Chan 1994; JOINT1 adds path-shared shared-τ; DRIFT1 adds batch linear α). **No** RF fingerprint training. **No** fingerprint rescue.
- Path must stay **laptop-feasible** sim. Hardware **X** is **PARKED**.
- Metric: **median** error on a held-out path inside a GPS-denied box. Provisional **sim X = 0.50 m** (**JOINT1 fixed-offset + named DRIFT1 batch α + NLOS-scoped**; **median-not-p90**; p90 ≈ 1.16 m @ 1 ns LOS is honesty, not the bar). Hardware **X** stays parked.
...
## Next (GATE1 — not this fold)

**GATE1**, then a Greer-facing write-up. Named, not run here. Still **no RF / ML**. **Multipath later.** Fog peek + later gates: [`PROPOSED_PULSE.md`](PROPOSED_PULSE.md).

## Reading order

1. [`STATUS.md`](STATUS.md) — where we are
2. [`NAMED_GAP_LEDGER.md`](NAMED_GAP_LEDGER.md) — the open / parked / hardened / Soften lines
3. [`SCORE_DRIFT1.md`](SCORE_DRIFT1.md) — DRIFT1 gated metrics
4. [`DIGESTION_DRIFT1.md`](DIGESTION_DRIFT1.md) — what DRIFT1 taught
5. [`SCORE_JOINT1.md`](SCORE_JOINT1.md) — JOINT1 gated metrics (prior Soften)
6. [`DIGESTION_JOINT1.md`](DIGESTION_JOINT1.md) — what JOINT1 taught
7. [`SCORE_SYNC_1.md`](SCORE_SYNC_1.md) — SYNC1 gated metrics (prior Soften)
8. [`DIGESTION_SYNC_1.md`](DIGESTION_SYNC_1.md) — what SYNC1 taught
9. [`SCORE_MULTIPATH1.md`](SCORE_MULTIPATH1.md) — MULTIPATH1 gated metrics (prior Soften)
10. [`DIGESTION_MULTIPATH1.md`](DIGESTION_MULTIPATH1.md) — what MULTIPATH1 taught
11. [`SCORE_GEOMETRY_0.md`](SCORE_GEOMETRY_0.md) — #0 gated metrics
12. [`DIGESTION_GEOMETRY_0.md`](DIGESTION_GEOMETRY_0.md) — what #0 taught
13. [`PROPOSED_PULSE.md`](PROPOSED_PULSE.md) — Lab fog peek + later gates
14. [`DIGESTION_FIRST_PULSE.md`](DIGESTION_FIRST_PULSE.md) — what the peek taught
15. [`DIGESTION_FROM_CELL_TOWER.md`](DIGESTION_FROM_CELL_TOWER.md) — what the last string taught
