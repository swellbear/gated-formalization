...
**What this does *not* mean:** A TDOA locator. Claim clearance. A remake of commercial RTLS. A multipath-robust 0.50 m. Hardware **X**. Training started or established. Skill-met. RF fingerprinting. GPS/DGPS as the mobile fix. A product copied from US10135667B1. Inventing the cheap-check options on this fold. Starting multipath wave-2 now. Reopening cell-tower as live. Reopening BIA→weight. Reopening Track B. Reopening llm-gwt R-REPL. Reopening Collatz invent (#45 playground is done; Lab HOLD there).
...
| sync-fragility-solve-target | (A) named Soften that keeps **sim X = 0.50 m** when `σ_sync` goes beyond 0.3 ns / drift **without** fingerprint / ML **or** (B) detect from measurements alone and Soften / widen **X** / refuse a point fix | Empirically posed; invent board authorized | **open** / **REOPEN** — last check: 2026-09-05 Founder / Operator REOPEN; invent board **not run**; Lab HOLD lifted for this board only |
...
| 2026-09-05 | Founder / Operator **REOPEN** sync-fragility solve-target (solve framing — **not** park forever). **(A)** keep provisional sim X=0.50 m under named sync Soften (`σ_sync` beyond 0.3 ns / drift) without fingerprint/ML invent **or (B)** detect from measurements alone when sync left the near-ideal band and Soften / widen X / refuse a point fix. DIGEST baseline **preserved**. GPS refs only. No US10135667B1 claim-language product copy. **Not** a remake of commercial RTLS. Lab HOLD **lifted for invent board only**. First pulse after REOPEN (**not this fold**): Lab invents 2–3 cheap-check options; Founder ranks. Multipath wave-2 after sync string clears or parks. Hardware **X** stays **PARKED**. End artifact later: plain-English write-up for Greer with pulse evidence. Cell-tower stays **PARKED**. BIA stays **CLOSED**. REOPEN ≠ claim clearance. Not a locator. Not skill-met. Not rithm. |
