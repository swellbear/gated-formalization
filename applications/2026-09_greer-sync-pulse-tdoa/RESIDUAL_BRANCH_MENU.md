...
**Open string after Founder / Operator REOPEN.** Offering ≠ running. This is **not** a closeout menu, **not** a locator, and **not** a product claim.

**Date:** 2026-09-05  
**Application:** `2026-09_greer-sync-pulse-tdoa`  
**Status:** fog peek **ADMITTED**; **GEOM0 HARDEN**; **MULTIPATH1 Soften**; **SYNC1 Soften**; Founder / Operator **REOPEN** sync-fragility solve-target; DIGEST baseline **preserved**; provisional **sim X = 0.50 m** remains under **sync + NLOS** scope (**median**-not-p90); hardware **X PARKED**; Lab HOLD **lifted for invent board only**
...
**What we’re doing:** Founder / Operator **REOPEN** the sync-fragility leftover as a solve-target (not park forever). **Collaboration-with-owner:** Greer (patent owner) **requested** they solve his problem. Deliverable is **for him by request**, **not** an unsolicited write-up. **Do not** invent product claim-language copy. Founder **may re-aim** when Greer’s success criteria arrive. Baseline standing stays: GEOM0 HARDEN; MULTIPATH1 Soften (mild-NLOS); SYNC1 Soften (`σ_sync ≲ 0.3 ns`). DIGEST baseline **preserved**. Sim **X** stays **0.50 m**, honest only under near-ideal sync and LOS / mild/intermittent NLOS. Hardware **X** stays parked. Lab HOLD lifted **only** for the (A)/(B) invent board. **No RF / ML.** **Not** a remake of commercial RTLS.

**What we need from you:** Nothing this fold. Next pulse (not run here): Lab invents 2–3 cheap-check options for (A)/(B); Founder ranks. Multipath wave-2 after the sync string.

**What this does *not* mean:** A TDOA locator. Claim clearance. A remake of commercial RTLS. An unsolicited write-up. Invented product claim-language. A multipath-robust 0.50 m. Hardware **X**. Training started or established. Skill-met. RF fingerprinting. GPS/DGPS as the mobile fix. A product copied from US10135667B1. Inventing the cheap-check options on this fold. Starting multipath wave-2 now. Reopening cell-tower as live. Reopening BIA→weight. Reopening Track B. Reopening llm-gwt R-REPL. Reopening Collatz invent (#45 playground is done; Lab HOLD there).
...
| sync-fragility-solve-target | (A) named Soften that keeps **sim X = 0.50 m** when `σ_sync` goes beyond 0.3 ns / drift **without** fingerprint / ML **or** (B) detect from measurements alone and Soften / widen **X** / refuse a point fix | Empirically posed; invent board authorized | **open** / **REOPEN** — last check: 2026-09-05 Founder / Operator REOPEN; invent board **not run**; Lab HOLD lifted for this board only; **kept** until Founder re-aims |
| greer-success-criteria | Greer’s owner-request success criteria (not yet arrived) | Awaiting owner | **open** / **awaiting owner criteria** — Founder **may re-aim** when they arrive; do **not** invent product claim-language in the meantime |
| reopen-invent-board | Lab invents 2–3 cheap-check options for (A)/(B); Founder ranks | Authorized; not this fold | **open** / **awaiting next pulse** — **not** commercial RTLS remake; **no** RF / ML |
| multipath-wave-2 | Later textbook pulse after the sync string clears or parks | Parked until sync string clears or parks | **paused** / **PARKED** — do not start now |
...
| 2026-09-05 | Operator **ADMIT SYNC1 Soften** (Kill **not** triggered). Frozen Chan 1994; `σ_t`=1 ns; same refs/L-path. Near-ideal `σ_sync≲0.3 ns` → median **0.382 m** ≤ X; `σ_sync`=1 ns scrapes **0.513 m**; `≥3 ns` / 3 ns path drift **fails X**. Combined **X** scope = near-ideal sync + prior MULTIPATH1 NLOS scope. **GEOM0 HARDEN** still stands. Median-not-p90 honesty remains (1 ns p90 ≈ **1.16 m**). No fingerprint / ML / RF invent to rescue loose sync. **PARK** hardware **X**. **HOLD** at gate close — optional later textbook pulses parked until Founder/user reopens. Founder DIGEST recorded on [`DIGESTION_SYNC_1.md`](DIGESTION_SYNC_1.md). US10135667B1 = custom-beacon substrate, not carrier-mast Amb. Cell-tower stays **PARKED**. BIA stays **CLOSED**. Soften ≠ claim clearance. Not a locator. Not skill-met. Not rithm. |
| 2026-09-05 | Founder / Operator **REOPEN** sync-fragility solve-target (solve framing — **not** park forever). **Collaboration-with-owner:** Greer (patent owner) **requested** they solve his problem. Deliverable is **for him by request**, **not** an unsolicited write-up. **Do not** invent product claim-language copy. **(A)** keep provisional sim X=0.50 m under named sync Soften (`σ_sync` beyond 0.3 ns / drift) without fingerprint/ML invent **or (B)** detect from measurements alone when sync left the near-ideal band and Soften / widen X / refuse a point fix. Founder **may re-aim** when Greer’s success criteria arrive. DIGEST baseline **preserved**. GPS refs only. **Not** a remake of commercial RTLS. Lab HOLD **lifted for invent board only**. First pulse after REOPEN (**not this fold**): Lab invents 2–3 cheap-check options; Founder ranks. Multipath wave-2 after sync string clears or parks. Hardware **X** stays **PARKED**. Cell-tower stays **PARKED**. BIA stays **CLOSED**. REOPEN ≠ claim clearance. Not a locator. Not skill-met. Not rithm. |
