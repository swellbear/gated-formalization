...
**Open string after MULTIPATH1 Soften.** Offering ≠ running. This is **not** a closeout menu, **not** a locator, and **not** a product claim.

**Date:** 2026-09-05  
**Application:** `2026-09_greer-sync-pulse-tdoa`  
**Status:** fog peek **ADMITTED**; **#0 geometry HARDEN**; **MULTIPATH1 Soften**; provisional **sim X = 0.50 m** **NLOS-scoped** (**median**-not-p90); hardware **X PARKED**
...
**What we’re doing:** Recording the Method Operator **MULTIPATH1 Soften**. Kill did **not** fire. Sim **X** stays **0.50 m** with an **NLOS scope annotation**. **X** remains **median-not-p90**. Hardware **X** stays parked. Next pulse (**admitted**, not this fold) = **sync-imperfection**. **No RF / ML.** **No** fingerprint rescue.

**What we need from you:** **none for this Soften.** Lab HOLD on running sync-imperfection here.
...
|----|----------|-------|-------------|
| greer-tdoa-median-X | ≥3-ref simultaneous-sync TDOA, laptop-feasible sim-only, **median** error ≤ **0.50 m** on a held-out GPS-denied path; GPS/DGPS place/time refs only; no RF fingerprint training | Empirically posed on sim (provisional **X** locked, **NLOS-scoped**, **median-not-p90**); succeed still requires a later held-out median ≤ X **without** dropping honesty locks | **open** / **Soften** — last check: MULTIPATH1 Soften; **sim X = 0.50 m** NLOS-scoped; **not** claim clearance; **not** multipath-robust |
...
| sync-imperfection | Does the NLOS-scoped 0.50 m sim bar survive named clock / sync error? | Empirically resolvable on the **admitted** next pulse | **open** / **admitted as next pulse, not run** — **not this fold**; Lab does **not** run it here |
...
| 2026-09-05 | Operator **ADMIT MULTIPATH1 Soften**. Kill **not** triggered. Frozen Chan 1994; `σ_t`=1 ns; positive range-bias injection; same refs/L-path as #0. Baseline median **0.364 m**; `random_k=1` `b=0.5` → **0.476 m**; `epoch_f=0.25` `b=1` → **0.452 m**; strong persistent `b≥1–2 m` → **0.73–4.7+ m**. **LOCK** provisional **sim X = 0.50 m** remains, with **NLOS scope annotation** (poseable under LOS + mild/intermittent NLOS; **not** poseable under strong persistent multipath with frozen Chan alone). Do **not** claim multipath-robust 0.50 m. **No** fingerprint rescue. **X** remains median-not-p90 (p90 ≈ **1.16 m** @ 1 ns LOS). **PARK** hardware **X**. Next (**admitted**, not this fold): **sync-imperfection**. US10135667B1 prior-art note only. Cell-tower stays **PARKED**. BIA stays **CLOSED**. Lab scratch not on this VM; metrics copied from the Operator gate into [`SCORE_MULTIPATH1.md`](SCORE_MULTIPATH1.md). Soften ≠ claim clearance. TDOA locate is **not** established. Training is **not** established. Not skill-met. Not rithm. |
