...
**Open string after DRIFT1 HARDEN.** Offering ≠ running. This is **not** a closeout menu, **not** a locator, and **not** a product claim.

**Date:** 2026-09-05  
**Application:** `2026-09_greer-sync-pulse-tdoa`  
**Status:** fog peek **ADMITTED**; **GEOM0 HARDEN**; **MULTIPATH1 Soften**; **SYNC1 Soften**; **JOINT1 Soften**; **DRIFT1 HARDEN**; provisional **sim X = 0.50 m** remains under **JOINT1 fixed-offset + named DRIFT1 batch α + NLOS** scope (**median**-not-p90); hardware **X PARKED**; **GATE1** named next (not run)
...
**What we’re doing:** Recording DRIFT1 HARDEN after GEOM0 HARDEN, MULTIPATH1 Soften, SYNC1 Soften, and JOINT1 Soften. Batch path-shared τ + linear α restores median ≤ **0.50 m** on SYNC1 drift breakers (drift=3 @ `σ=0` → **0.221 m**; drift=10 → **0.223 m**; **α̂ recovers**). Sim **X** stays **0.50 m**, honest under `σ_sync ≲ 3 ns` **under JOINT1 (fixed offsets)** + named DRIFT1 batch α + LOS / mild/intermittent NLOS. **X is median-not-p90** (1 ns p90 ≈ **1.16 m**). Hardware **X** stays parked. **No RF / ML.**

**What we need from you:** Nothing this fold. Next named pulse is **GATE1**, then a Greer-facing write-up (not run here). Multipath later.

**What this does *not* mean:** A TDOA locator. Claim clearance. A multipath-robust 0.50 m. A free per-epoch realtime drift claim. Hardware **X**. Training started or established. Skill-met. RF fingerprinting. GPS/DGPS as the mobile fix. A product copied from US10135667B1. Auto-run of GATE1. Reopening cell-tower as live. Reopening BIA→weight. Reopening Track B. Reopening llm-gwt R-REPL. Reopening Collatz invent (#45 playground is done; Lab HOLD there).
...
|----|----------|-------|-------------|
| greer-tdoa-median-X | ≥3-ref simultaneous-sync TDOA, laptop-feasible sim-only, **median** error ≤ **0.50 m** on a held-out GPS-denied path; GPS/DGPS place/time refs only; no RF fingerprint training | Empirically posed on sim (**X** remains, **scoped** to `σ_sync ≲ 3 ns` under JOINT1 **fixed offsets** + named DRIFT1 batch α + mild NLOS; **median-not-p90**; **not** free per-epoch realtime); succeed still requires a later held-out median ≤ X **without** dropping honesty locks | **open** — last check: DRIFT1 HARDEN; **sim X = 0.50 m** scoped; 1 ns p90 ≈ **1.16 m**; **not** claim clearance |
...
| hardware-X | A hardware / spectrum **X** on a later radio path | Parked — live path is sim-only | **paused** / **PARKED** — last check: 2026-09-05 Operator **PARK** hardware **X** (DRIFT1 does not unpark) |
...
| sync-imperfection | Does the NLOS-scoped 0.50 m sim bar survive named clock / sync error? | Scored Soften (Chan-alone + JOINT1) | **Soften** — last check: 2026-09-05 Operator **JOINT1 Soften**; Chan-alone SYNC1 `σ_sync≲0.3 ns` **0.382 m**; JOINT1 fixed_trial restores X to **≲ 3 ns** (**0.231** @ 1 ns; **0.439** @ 3 ns); 10 ns **1.816 m** fails; **still stands** |
| joint1-path-shared | Path-shared joint clocks (shared-τ) under fixed_trial `σ_sync` | Scored Soften | **Soften** — last check: 2026-09-05 Operator **JOINT1 Soften**; Aim A partial; Chan scrape at 1 ns restored; **fixed offsets**; **still stands** |
| drift1-pulse | Clock drift / ramp vs shared-τ | Scored HARDEN | **hardened** — last check: 2026-09-05 Operator **DRIFT1 HARDEN**; batch path-shared τ + linear α; drift=3 @ `σ=0` **0.221 m**; drift=10 **0.223 m**; **α̂ recovers**; **not** free per-epoch realtime |
| gate1-pulse | Named next after DRIFT1, then a Greer-facing write-up | Named next | **open** / **named** — last check: none; **GATE1** not run this fold |
