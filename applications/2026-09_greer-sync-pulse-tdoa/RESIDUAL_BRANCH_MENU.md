...
**Open string after GATE1 Soften + write-up on disk (HOLD send).** Offering ≠ running. This is **not** a closeout menu, **not** a locator, and **not** a product claim.

**Date:** 2026-09-05  
**Application:** `2026-09_greer-sync-pulse-tdoa`  
**Status:** fog peek **ADMITTED**; **GEOM0 HARDEN**; **MULTIPATH1 Soften**; **SYNC1 Soften**; **JOINT1 Soften**; **DRIFT1 HARDEN**; **GATE1 Soften**; provisional **sim X = 0.50 m** remains under **JOINT1 fixed-offset + named DRIFT1 batch α + GATE1 refuse-belt + NLOS** scope (**median**-not-p90); hardware **X PARKED**; write-up on disk (**HOLD send**); Lab **HOLD** invent
...
**What we’re doing:** Recording GATE1 Soften after DRIFT1 HARDEN. Detect-only refuse OR (G1a residual ∨ G1b raw LORO): FA ≈ **0.10** in-band; TD high out-of-band. Write-up is on disk. Founder [`GREER_WRITEUP.md`](GREER_WRITEUP.md) is **PRIMARY** — **HOLD send** until user OK. Lab **HOLD** invent. Hardware **X** stays parked. **No RF / ML.**

**What we need from you:** User send OK (or hold). Lab does **not** invent next. Multipath later.

**What this does *not* mean:** A TDOA locator. Claim clearance. A send to Greer. A multipath-robust 0.50 m. A free per-epoch realtime drift claim. A GATE1 accuracy repair. Hardware **X**. Training started or established. Skill-met. RF fingerprinting. GPS/DGPS as the mobile fix. A product copied from US10135667B1. Reopening cell-tower as live. Reopening BIA→weight. Reopening Track B. Reopening llm-gwt R-REPL. Reopening Collatz invent (#45 playground is done; Lab HOLD there).
...
|----|----------|-------|-------------|
| greer-tdoa-median-X | ≥3-ref simultaneous-sync TDOA, laptop-feasible sim-only, **median** error ≤ **0.50 m** on a held-out GPS-denied path; GPS/DGPS place/time refs only; no RF fingerprint training | Empirically posed on sim (**X** remains, **scoped** to `σ_sync ≲ 3 ns` under JOINT1 **fixed offsets** + named DRIFT1 batch α + GATE1 refuse-belt + mild NLOS; **median-not-p90**; **not** free per-epoch realtime); succeed still requires a later held-out median ≤ X **without** dropping honesty locks | **open** — last check: GATE1 Soften; **sim X = 0.50 m** scoped; 1 ns p90 ≈ **1.16 m**; write-up **HOLD send**; **not** claim clearance |
...
| gate1-pulse | Detect-only refuse OR (G1a_DRIFT1 residual ∨ G1b raw LORO) | Scored Soften | **Soften** — last check: 2026-09-05 Operator **GATE1 Soften**; FA≈**0.100**; TD σ=10 ≈**0.828**; residual-alone misses σ=10; raw LORO carries it; **not** a repair |
