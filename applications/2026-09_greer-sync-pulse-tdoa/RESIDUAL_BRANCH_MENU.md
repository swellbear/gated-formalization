...
**Open string after JOINT1 Soften.** Offering ≠ running. This is **not** a closeout menu, **not** a locator, and **not** a product claim.

**Date:** 2026-09-05  
**Application:** `2026-09_greer-sync-pulse-tdoa`  
**Status:** fog peek **ADMITTED**; **GEOM0 HARDEN**; **MULTIPATH1 Soften**; **SYNC1 Soften**; **JOINT1 Soften**; provisional **sim X = 0.50 m** remains under **JOINT1 sync + NLOS** scope (**median**-not-p90); hardware **X PARKED**; **DRIFT1** named next (not run)
...
**What we’re doing:** Recording JOINT1 Soften after GEOM0 HARDEN, MULTIPATH1 Soften, and SYNC1 Soften. Kill is **not** triggered. Aim A is **partial**. Sim **X** stays **0.50 m**, honest under `σ_sync ≲ 3 ns` **under JOINT1** (path-shared / fixed_trial) and LOS / mild/intermittent NLOS. Drift 3 ns/path still fails. **X is median-not-p90** (1 ns p90 ≈ **1.16 m**). Hardware **X** stays parked. **No RF / ML.**

**What we need from you:** Nothing this fold. Next named pulse is **DRIFT1** (not run here).

**What this does *not* mean:** A TDOA locator. Claim clearance. A multipath-robust 0.50 m. A drift-robust 0.50 m. Hardware **X**. Training started or established. Skill-met. RF fingerprinting. GPS/DGPS as the mobile fix. A product copied from US10135667B1. Auto-run of DRIFT1. Reopening cell-tower as live. Reopening BIA→weight. Reopening Track B. Reopening llm-gwt R-REPL. Reopening Collatz invent (#45 playground is done; Lab HOLD there).
...
|----|----------|-------|-------------|
| greer-tdoa-median-X | ≥3-ref simultaneous-sync TDOA, laptop-feasible sim-only, **median** error ≤ **0.50 m** on a held-out GPS-denied path; GPS/DGPS place/time refs only; no RF fingerprint training | Empirically posed on sim (**X** remains, **scoped** to `σ_sync ≲ 3 ns` under JOINT1 + mild NLOS; **median-not-p90**; **not** drift-robust); succeed still requires a later held-out median ≤ X **without** dropping honesty locks | **open** — last check: JOINT1 Soften; **sim X = 0.50 m** scoped; 1 ns p90 ≈ **1.16 m**; **not** claim clearance |
...
| hardware-X | A hardware / spectrum **X** on a later radio path | Parked — live path is sim-only | **paused** / **PARKED** — last check: 2026-09-05 Operator **PARK** hardware **X** (JOINT1 does not unpark) |
...
| sync-imperfection | Does the NLOS-scoped 0.50 m sim bar survive named clock / sync error? | Scored Soften (Chan-alone + JOINT1) | **Soften** — last check: 2026-09-05 Operator **JOINT1 Soften**; Chan-alone SYNC1 `σ_sync≲0.3 ns` **0.382 m**; JOINT1 fixed_trial restores X to **≲ 3 ns** (**0.231** @ 1 ns; **0.439** @ 3 ns); 10 ns **1.816 m** fails |
| joint1-path-shared | Path-shared joint clocks (shared-τ) under fixed_trial `σ_sync` | Scored Soften | **Soften** — last check: 2026-09-05 Operator **JOINT1 Soften**; Aim A partial; Chan scrape at 1 ns restored; **not** drift-robust |
| drift1-pulse | Clock drift / ramp vs shared-τ | Named next | **open** / **named** — last check: JOINT1 drift 3 ns/path **0.919 m** fails X; **DRIFT1** not run this fold |
...
| 2026-09-05 | Operator **ADMIT JOINT1 Soften** (Kill **not** triggered; Aim A **partial**). Path-shared joint clocks (shared-τ) under **fixed_trial** `σ_sync`. Restores X up to **≲ 3 ns**: **0.231 m** @ 1 ns; **0.439 m** @ 3 ns ≤ X. Chan scrape at 1 ns **restored**. `σ_sync`=10 ns fails (**1.816 m**). Drift 3 ns/path still breaks X (JOINT1 **0.919 m** — shared-τ misspecified vs ramp). Named sync Soften budget **widens** to **`σ_sync ≲ 3 ns` under JOINT1** + prior mild-NLOS. **GEOM0 HARDEN** + **MULTIPATH1 Soften** still stand. **Not** multipath-robust. **Not** drift-robust. Median-not-p90 honesty remains (1 ns p90 ≈ **1.16 m**). No fingerprint / ML / RF invent. **PARK** hardware **X**. Next (**not this fold**): **DRIFT1**. Owner-requested **collaboration framing**; **no claim-language copy**. US10135667B1 = custom-beacon substrate, not carrier-mast Amb. Cell-tower stays **PARKED**. BIA stays **CLOSED**. Soften ≠ claim clearance. Not a locator. Not skill-met. Not rithm. |
