...
**Quick map:** Phase 1 (autonomous while Amb drops / gaps descriptive) → Phase 2 (authorized; scientific/math/technical) → optional Experimental Generation (batch-authorized) → closeout with **Original-Claim Assessment** → optional **Claim-Revision Scaffolding** (authorized revision path only) → optional **Quantitative Implication & Counterfactual Benefit** (after a failed numerical instance test).  
Also in force when applicable: inter-parameter dependencies; locking-scaffolding (plain-language framing; ranked packages; **objective claim-deviation assessment**; **forced-deviation extraction** when no Minimal-deviation package exists; relevance warnings; **OR-slot resolution**; minimal choice prompt); scoped-result honesty; package evidence intake; compact no-admit readouts; revision-vs-continuation fork at closeout; Claim-Revision Scaffolding (ranked successor claims; non-silent); Quantitative Implication & Counterfactual Benefit (scale-factor + optional benefit evaluation; distinct from Experimental Generation and Claim-Revision Scaffolding).  
...
- [ ] Copy templates `01`–`05` (+ `R_Locking_Scaffolding`, `CR_Claim_Revision_Scaffolding`, `QI_Quantitative_Implication_Counterfactual_Benefit`, `E_Package_Evidence_Intake`, `Compact_No_Admit_Readout` as needed)
...
- [ ] Package evidence → `E_Package_Evidence_Intake.md` then `04`
...
## 8. Close the Run
- [ ] Fill `05_Original_Claim_Assessment.md` (status + continuation options + **revision vs continuation fork** when strong language is mostly negatively constrained)
