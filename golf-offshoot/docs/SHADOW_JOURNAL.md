...
| settle_source | Optional. Where the join came from (`paper_ledger_ticket`, `espn_official_final`, `paper_book_official_winner`, or a loud `never_settled` reason such as `espn_official_final:absent_from_official_field`, `espn_official_final:finish_unknown`, `espn_official_final:round_leader_not_72_hole`) |
...
2. Official ESPN-shaped inspect (`completed` and exactly one official winner / STATUS_FINAL field), including `{artifact_root}/settlements/{event_id}.json` or `--join-settles`: same `ticket_hit` rule as `paper-settle`. Round-leader markets and unknown finishes (`player_id` on the field but `place` is null) → `never_settled`. **Absent from official field:** if the advise `player_id` is not on that official STATUS_FINAL finisher list, mark `settle_status=never_settled` with `settle_source=espn_official_final:absent_from_official_field`. Do **not** invent `paper_win` / `paper_lose`. Unofficial / playoff / inspect failure → leave unset (`SETTLE_PENDING`). Source `espn_official_final` when a real finish is scored. Cached inspects may live at `{artifact_root}/settlements/{event_id}.json`.
...
`SETTLE_PENDING` on the operator surface **clears only when every relevant advise** (`win` / `top_5` / `top_10` / `top_20` / `make_cut`) is `paper_win` or `paper_lose`. `never_settled` and missing `settle_status` keep the weekly operating claim blocked, **except** `never_settled` with `settle_source=espn_official_final:absent_from_official_field` (player_id absent from the official STATUS_FINAL finisher list). Those rows stay visible in the shadow journal / honesty strip and are **excluded from the banner denominator** — they must not keep the weekly claim blocked. Applies to place markets and to `win` if the same absence exists. Short token `absent_from_official_field` is accepted on disk; the join writes the `espn_official_final:`-prefixed form.
