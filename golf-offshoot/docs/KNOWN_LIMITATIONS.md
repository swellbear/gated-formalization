...
- **New course setups / renovations** with little history: course-fit and course-history stay unconstrained; venue-cluster borrow is a shrink toward *similar* courses, not knowledge of a new cut of rough. The ticket screen does not zero the field for that gap (`course_history_missing` is display/Rel only). Player-level `sparse_data` still hard-passes.
...
- **Market odds** use The Odds API when a key and golf outright exist; otherwise Bovada public Winner / Winner Live, plus Finishes cards when listed on the matching event. Live passes refetch with a 45s TTL. Failed refresh uses a labeled stale snapshot only if younger than 15 minutes; older prices are suppressed. Edges are vs proportional de-juice; Winner tickets still have to beat the posted decimal by 3pp. US end-of-round leader uses a scaled vs-Yes bar (not the Winner 3pp exam) and is not scored as 72-hole Win. Unmatched names are unavailable, not invented. `--book polymarket` reads Polymarket **US** golf futures (Winner and end-of-round leader when listed on `gateway.polymarket.us`). International Gamma Top 5/10/20 cards are not ingested and are not taken from Bovada. Opening lines are stored only when a distinct prematch coupon exists (live coupon and/or `data/openings/` archive). Current in-play prices are never claimed as opens. Champions Tour finishes are not attached to PGA events. Weekend paper settle voids still-open round-leader tickets at cost (they are not 72-hole finishes).
...
- **Operator leftover callout** (used vs unconstrained vs held-ticket residual, plus round-leader vs-posted when a Yes quote exists, plus flip heat P(early contract % ≥ ask+spread) on listed Yes) prints after `ingest` / `live` / pressure-test. Display only. Flip NEW is a ticket suggestion only when that P is at least 0.20, one per player, and only while fewer than 6 flip tickets are already open (a later live does not top up with the next-best names; a fail/sell can free a slot). Unlisted Polymarket cards are not invented. It does not fill agronomy, tee pairing, narrative, or injury rumor. Spec: [PARKED_LEFTOVER_CALLOUT.md](PARKED_LEFTOVER_CALLOUT.md).
