...
- **Search book:** YES-or-skip. Never buy NO. Family 1 is skip-rich-YES. Family 2 (skip-wide-spread) hunts **in parallel** on the same sidecar tick, not after family-1 clip/starve. The default process dates unused clip starts in one fire (family 1: every 0.02 from 0.55 through 0.90 inclusive; family 2: every 0.01 from 0.02 through 0.12 inclusive). File order is the exam queue. Dating is not a trial. Palshi does not re-press this.
...
- **Exam book:** lived fills at **frozen** knobs when freeze *f* fires. Own k (in `data/honer_15m/latest/trials.json`). n=70. **One exam chair for all hunts.** Not a keep.
- **Freeze f `in_band_v1`:** ≥20 **in-band** settled search windows, `|θ − last_declared_θ| ≥ 0.05` (or `|δ − last_declared_δ| ≥ 0.02` on the spread family), **and** 5 consecutive **in-band** windows with no knob move. Far tickets increment an ignored counter only. Retired or spent vectors cannot freeze. First freeze-ready brain in file order takes the chair and increments Honer k once.
...
- **8765 HTML and the local honer PNG share columns:** Near line / Spread / Wide-book. Freeze meter names the file-order chair (not the closest hunt). The search table is the **canonical** hunt, labeled not-a-sum. Lineage A PNG is untouched.
- **Library:** `latest/library.json`. Outcomes are `parked`, `completed_dead`, `completed_unscored`, or `search_untestable`. Labels compound. Pnl does not rank. Not a score. `search_untestable` does **not** increment honer exam k. When `catalog_exhausted` is true, **search is parked** (no new search fills). Exam stays closed. `latest/family_amend.json` is the file doorbell that `HONER-FAMILY-AMEND` is owed (`catalog_exhausted` or exam `completed_dead`). It does not read exam pnl and does not date a third family.
- **Catalog:** [`HONER_15M_CATALOG.json`](HONER_15M_CATALOG.json). Two families only, both `activate: start`. They hunt in parallel on one quote-bus tick. Incomplete quotes → skip that spread hunt this tick; do not invent a spread. Clip menu: [`HONER_15M_SEARCH_BRAINS.json`](HONER_15M_SEARCH_BRAINS.json). Amend protocol: [`HONER_15M_CATALOG_AMEND.md`](HONER_15M_CATALOG_AMEND.md).
...
- **Search starvation (honer only):** on each hunt's clock `search_settled_since_freeze`, look at n=40 (starve if zero in-band visits) and n=70 (starve if in-band < 20). Starve retires that vector as `search_untestable` and resets that hunt to its dated start. It does **not** increment exam k and does **not** start the other family (already parallel). Not a factory rule. Not consult enable. Deploy does not zero live search clocks.
