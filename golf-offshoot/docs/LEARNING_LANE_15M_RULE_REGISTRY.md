...
- `R-SKIP-2TO1-FAVORITE` — second selection rule (PROPOSED 02, 2026-09-08 16:53 EDT): skip when posted YES is a 2-to-1 or better favorite (`favorite_odds=2` ⇒ `p=2/3`). The free parameter is the integer prior, not a tape cut. **RUN-ONLY** by Operator 2026-09-08 17:11 EDT; `execution=true` (paper only). `decide()` expresses it from `params.favorite_odds`. Do not score it in the proposing or authorizing turn. Do not retune `favorite_odds` from the tape. Not an ADMIT.
