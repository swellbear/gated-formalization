...
**Evidence bar:** draft at [`LEARNING_LANE_15M_EVIDENCE_BAR.md`](LEARNING_LANE_15M_EVIDENCE_BAR.md) — **not binding**. `trials_to_date` started at 0 and is now **2** (declaration of `R-SKIP-2TO1-FAVORITE`, then `R-SKIP-HOUR-CLOSE`).
...
- `R-SKIP-2TO1-FAVORITE` — second selection rule (PROPOSED 02, 2026-09-08 16:53 EDT): skip when posted YES is a 2-to-1 or better favorite (`favorite_odds=2` ⇒ `p=2/3`). **RUN-ONLY** by Operator 2026-09-08 17:11 EDT; L1 PARK on the falsifier 2026-09-10. `execution` dropped 2026-09-10 13:36 so PROPOSED 03 could take the only selecting seat. `favorite_odds=2` is **not verifiably pre-registered**. Do not retune `favorite_odds` from the tape. Do not re-score. Not an ADMIT.
- `R-SKIP-HOUR-CLOSE` — third selection rule (PROPOSED 03, 2026-09-10 13:25 EDT): skip when `close_at` clock minute equals `0` (`skip_close_minute=0`). **RUN-ONLY** by Operator 2026-09-10 13:36 EDT; L1 **PARK** on the registry falsifier 2026-09-12. `execution` dropped. Gym `operator_look=PARK` 2026-09-12 07:37. First naming `c917e56`. Not a posted-yes cut. Do not retune the minute from the tape. Do not re-score. Not an ADMIT.
