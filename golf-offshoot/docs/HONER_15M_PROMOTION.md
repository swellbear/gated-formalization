# honer_15m promotion protocol

**This file is not an ADMIT, not a keep, and not a consult enable.**
...
- Factory fill + honer fill → fill.
- Snapshot absent or `consult_enabled` not exactly `true` → no-op. Lineage A matches `rules.decide()` alone.
- Live wandering `honer_15m/latest/theta.json` never consults.
- `paper.py` / `rules.py` do not import `honer_15m`. Honer joins `_CODE_PKG_DIRS` only when the consult is enabled.

**Replacement** (surviving freeze becomes the executing registry row) is crew: Lab dates the freeze knobs as the executing row, Operator RUN-ONLY, no ADMIT, not arm. Do not steal a sitting factory trial. Skip-on-mark may still fail Established on this dirty book; that is `declare_rule`, not a Founder stamp.

## Order

1. **Search** — in-band freeze (`in_band_v1`). Far tickets audit, they do not count. Family 2 and family 3 wait on clip exhaustion **or** second search-starvation, plus quote quality. Search starvation is a honer search give-up (`search_untestable`); it does not increment exam k and does not enable consult. Family 3 (`H-SKIP-THIN-BOOK`) is dated from files. Palshi does not ping Lab. Palshi does not CoS-assign Lab. A later Lab refusal still stands. Factory skip-together (`15m-3b`) is not this family. No fourth family until files date one.
...
If the exam parks or scores dead: notebook closed. Compositor stays dark. Factory `decide()` unchanged. A new family is `HONER-FAMILY-AMEND` dated from Honer files, not a tape sort. Palshi does not ping Lab. Operator RUN-ONLY and later Critic stay later sessions.
