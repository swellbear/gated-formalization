...
Operator surface owns a single selector field: `lane=golf|learning_lane_15m` (exact). No bare `15m` alias. Default is `golf`. Unknown → golf. Switching reloads that lane’s artifacts only.
