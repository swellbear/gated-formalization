...
**Amended:** 2026-09-09 10:35 EDT · Operator (ANSWER 11: scorer enforces the replay clock interval, not Lived; missing-fields default disclosed). Prior: 2026-09-09 09:05 EDT · Systems (lived/replay into `score_rule`; not a score).
...
**The scorer enforces the replay clock interval, not the table's Lived cell.** `window_is_oos` still keys only on `closed > declared_at` (the `decide()` expression gate). `score_rule` calls `_assert_window_lived_or_raise`, which reads `lived_paper_begins_at` / `replay_close_*` and raises `RuleNotScorable` on a window in `(replay_close_after, replay_close_at_or_before]`. A later score that treats a window from that interval as in-interval fails as a machine. The scorecard token `evidence=lived` means `closed > lived_paper_begins_at` (OOS and not in the named replay interval) and does **not** mean the loop honoured `decide()`. That token is a clock label. `lived_replay_enforced_by_scorer` is false for that reason. **Missing replay fields default every OOS close to that clock stamp:** a selecting row with no `replay_close_*` and no `lived_paper_begins_at` has every OOS close stamped `evidence=lived`. `R-SKIP-COINFLIP` is that row (`execution=false`; no interval fields). This turn does not score it, does not add fields to it, and does not open window outcome files.
...
| X2 reachable | `currently_reachable` stays false; `R-SKIP-2TO1-FAVORITE` registry `execution` is true; the scorecard `evidence=lived` token is a replay-clock label (`closed > lived_paper_begins_at`), not the table's Lived cell; lived honoring remains unproven on this tree this turn (no tip, no cited decision row); pre-registration / binding / fee hash / Founder read-once still block |
