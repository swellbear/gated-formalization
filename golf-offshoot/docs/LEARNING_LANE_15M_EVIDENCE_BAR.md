...
| gym-pin eighth | `series_fee_regime_matches` is in `CHECKS`; `snapshot_absent` (missing, unreadable, or non-dict) is `state: PASS` and will not block `passed: true` once the fee hash pins; a well-formed present missing/drifted M FAILs |
