...
Named on the face because fourteen sustained objections and zero new `CHECKS` members was the Y2 finding. The suite now has **eight** method checks (`series_fee_regime_matches` is the eighth). Several of the first seven were **rewritten** at `0a480d4` rather than appended. JSON copy: `ratchet_guards`.
