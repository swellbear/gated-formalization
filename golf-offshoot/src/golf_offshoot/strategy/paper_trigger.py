...
_MARKET_ORDER = {"Win": 0, "Top 5": 1, "Top 10": 2, "Top 20": 3, "Make cut": 4}
...
def write_trigger_files(
    ...
    *,
    ...
    html_path.write_text(
        _render_trigger_html(title=title, sections=sections),
        encoding="utf-8",
    )
    write_trigger_pdf(pdf, title=title, sections=sections)
    return TableExportPaths(pdf=pdf, html=html_path, txt=txt)


def _render_trigger_html(*, title: str, sections: list[TriggerSection]) -> str:
