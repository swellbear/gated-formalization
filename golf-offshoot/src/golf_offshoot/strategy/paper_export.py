...
def write_paper_book_files(
    ...
    *,
    ...
    subtitle = (
        ...
        f"run={record.locked_from_run_id or 'n/a'}   model={MODEL_VERSION}"
    )
    txt = d / f"{stem}.txt"
    html_path = d / f"{stem}.html"
    pdf = d / f"{stem}.pdf"
    txt.write_text(paper_book_document(record), encoding="utf-8")
    html_path.write_text(render_paper_html(record, title=title, subtitle=subtitle), encoding="utf-8")
    write_paper_pdf(pdf, record, title=title, subtitle=subtitle)
...
def paper_book_document(record: PaperBookFile) -> str:
    ...
    lines = [
        ...
        f"${record.bankroll:.0f} mock  |  {record.odds_book or 'book n/a'}  |  "
        f"locked {record.locked_at.isoformat()}  |  run={record.locked_from_run_id or 'n/a'}",
        ...
        observation_technical(),
        "",
        f"{'Player':<22} {'Mkt':<6} {'Stake':>8} {'Posted':>8} {'Model':>8} "
        f"{'EdgeW':>8} {'Vs post':>8} {'If wins':>10}  Screen",
    ]
    for t in ticket_rows(record):
        lines.append(
            f"{t.player_name:<22} {t.market:<6} ${t.stake:>7.2f} {t.posted:>8.2f} "
            f"{t.model_win * 100:>7.1f}% {t.edge_w * 100:>+7.1f}pp "
            f"{t.posted_edge * 100:>+7.1f}pp ${t.if_wins:>9.2f}  {t.screen}"
        )
    if not record.book.positions:
        lines.append("(no tickets locked)")
    lines.append("")
    lines.append("Vs posted = model win% minus 1/odds (the number you would actually buy).")
    lines.append(
        f"Ticket screen needs at least {MIN_EDGE_TO_CONSIDER * 100:.0f} percentage points "
        "on that posted-price gap, not only on EdgeW."
    )
    lines.append("If wins = stake times posted decimal odds (stake returned plus profit).")
    lines.append("Observation only. Never auto-bet.")
    return "\n".join(lines)


def render_paper_html(record: PaperBookFile, *, title: str, subtitle: str) -> str:
    rows_html = []
    for i, t in enumerate(ticket_rows(record)):
        ...
        rows_html.append(
            ...
            + stripe
            + ">"
            + f"<td class='txt'>{html.escape(t.player_name)}</td>"
            ...
            + f"<td class='txt'>{html.escape(t.screen)}</td>"
            + "</tr>"
        )
    cash = record.bankroll - record.book.open_exposure
    frac = (record.book.open_exposure / record.bankroll) if record.bankroll else 0.0
    body = "".join(rows_html) or "<tr><td colspan='9'>No tickets locked.</td></tr>"
...
<table class="field">
<thead><tr>
<th class="txt">Player</th><th class="txt">Market</th><th class="num">Stake</th>
<th class="num">Posted</th><th class="num">Model win</th><th class="num">EdgeW</th>
<th class="num">Vs posted</th><th class="num">If wins</th><th class="txt">Screen</th>
</tr></thead>
<tbody>
{body}
</tbody>
...
def write_paper_pdf(
    ...
    *,
    ...
    aligns = ("LEFT", "LEFT", "RIGHT", "RIGHT", "RIGHT", "RIGHT", "RIGHT", "RIGHT", "LEFT")
...
def _glossary() -> list[tuple[str, str]]:
    return [
        (
            "Posted",
            "The actual Bovada (or chosen book) decimal price you would buy. Not a fair/de-juiced number.",
        ),
        (
            "Model win",
            "The model's chance this player wins the tournament.",
        ),
        (
            "EdgeW",
            "Model win minus a fair market after stripping the book's extra juice. Can look better than the real ticket.",
        ),
        (
            "Vs posted",
            "Model win minus 1/posted odds. This is the gap versus the number you would actually buy.",
        ),
        (
            "Screen",
            f"Cleared only if Vs posted is at least {MIN_EDGE_TO_CONSIDER * 100:.0f} percentage points. "
            "Live juice often fails this even when EdgeW looks good.",
        ),
        (
            "If wins",
            "Stake times posted decimal odds (your stake comes back plus profit). Winner-only; no place ticket.",
        ),
        (
            "Paper",
...
def _paper_col_widths(epw: float) -> list[float]:
    weights = (0.14, 0.07, 0.08, 0.08, 0.09, 0.08, 0.09, 0.09, 0.28)
    total = sum(weights)
...
def export_existing_paper_book(event_id: str, *, directory: Path | None = None) -> TableExportPaths:
    ...
    return write_paper_book_files(record, directory=directory)
