...
def _hour_open_used(
    *,
    ...
    return _minutes_used({0, 15}, root=root, registry=registry, farm=farm)
...
def _hour_wrap_used(
    *,
    ...
    return _minutes_used({0, 45}, root=root, registry=registry, farm=farm)


def _intra_hour_used(
    *,
    root: Path | None = None,
    registry: dict[str, Any] | None = None,
    farm: dict[str, Any] | None = None,
...
def _except_first_quarter_used(
    *,
    root: Path | None = None,
    registry: dict[str, Any] | None = None,
    farm: dict[str, Any] | None = None,
) -> bool:
    return _minutes_used({0, 30, 45}, root=root, registry=registry, farm=farm)


def _except_last_quarter_used(
    *,
    root: Path | None = None,
    registry: dict[str, Any] | None = None,
    farm: dict[str, Any] | None = None,
) -> bool:
    return _minutes_used({0, 15, 30}, root=root, registry=registry, farm=farm)
    ...
    *,
    ...
    for kind in kinds:
        ...
        if kid == "CLOCK-HOUR-SECOND-HALF":
            ...
            continue
        if kid == "CLOCK-HOUR-OPEN":
            if kind.get("legal_now") is not True:
                continue
            if _hour_open_used(root=root, registry=registry, farm=payload):
                continue
            out.append(
                {
                    "kind": kid,
            ...
            continue
        if kid == "CLOCK-HOUR-WRAP":
            if kind.get("legal_now") is not True:
                continue
            if _hour_wrap_used(root=root, registry=registry, farm=payload):
                continue
            out.append(
                {
                    "kind": kid,
            ...
            continue
        if kid == "CLOCK-INTRA-HOUR":
            if kind.get("legal_now") is not True:
                continue
            if _intra_hour_used(root=root, registry=registry, farm=payload):
                continue
            out.append(
                {
                    "kind": kid,
        ...
        if kid == "CLOCK-EXCEPT-HALF-HOUR":
            ...
            continue
        if kid == "CLOCK-EXCEPT-FIRST-QUARTER":
            if kind.get("legal_now") is not True:
                continue
            if _except_first_quarter_used(root=root, registry=registry, farm=payload):
                continue
            out.append(
                {
                    "kind": kid,
            ...
            continue
        if kid == "CLOCK-EXCEPT-LAST-QUARTER":
            if kind.get("legal_now") is not True:
                continue
            if _except_last_quarter_used(root=root, registry=registry, farm=payload):
                continue
            out.append(
                {
                    "kind": kid,
